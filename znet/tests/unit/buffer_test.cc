...
TEST_F(BufferTest, Constructor) {
  ...
  EXPECT_EQ(buf.writable_bytes(), Buffer::kInitialSize);
  EXPECT_EQ(buf.prependable_bytes(), Buffer::kCheapPrepend);
}
...
TEST_F(BufferTest, AppendAndRead) {
  ...
  EXPECT_EQ(buf.writable_bytes(), Buffer::kInitialSize - len);
...
TEST_F(BufferTest, AutoGrow) {
  ...
  EXPECT_GE(buf.capacity(), large_data.size() + Buffer::kCheapPrepend);
}
